...
OpenXrViewport::OpenXrViewport(GpuInstance* gpu, OpenXrDisplay* display,
  ...
  auto swapchainCreateInfo = with(XrSwapchainCreateInfo,
...
void OpenXrViewport::writeCompositionLayers(
  ...
  projection_view->subImage = with(XrSwapchainSubImage, $.swapchain = swapchain);
  projection_view->subImage.imageRect = with(XrRect2Di,
      $.offset = {0, 0},
...
VkSemaphore OpenXrViewport::_acquireImage(uint32_t* image_index) {
  ...
  auto acquireInfo = with(XrSwapchainImageAcquireInfo,
      $.type = XR_TYPE_SWAPCHAIN_IMAGE_ACQUIRE_INFO, $.next = nullptr);

  xrAcquireSwapchainImage(swapchain, &acquireInfo, image_index);

  auto waitInfo = with(XrSwapchainImageWaitInfo, $.type = XR_TYPE_SWAPCHAIN_IMAGE_WAIT_INFO,
...
void OpenXrViewport::_releaseImage(uint32_t, VkSemaphore) {
  ...
  auto release_info = with(XrSwapchainImageReleaseInfo, 
